...
#[27-11-2016] Added the title in the redis
#[27-11-2016] Added progress bar
...
class isThatAWatch:
    def __init__(self, pathImage):
        ...
        REDIS_PORT = 6379 
        ASIN_POS = 0 #Scraped data: location in the redisDB of the ASIN
        TITLE_POS = 1 #Scraped data: location in the redisDB of the Title
...
#show a progress bar
widgets = ["Classification progress: ", progressbar.Percentage(), " ", progressbar.Bar(), " ", progressbar.ETA()]
pbar = progressbar.ProgressBar(maxval=len(listOfImages), widgets=widgets).start()
indexPbar = 0

for filename in (listOfImages):
    isThatAWatch(str(filename))
    indexPbar += 1
    pbar.update(indexPbar)
